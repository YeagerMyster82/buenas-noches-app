...
export function buildPlan({
  ...
  // Phases that happen BEFORE dinner (bath and getting ready)
  ...
  // If dinner falls in the routine window, split the routine around it
  // Steps before dinner end at dinner time; steps after dinner pick up after a 30-min dinner buffer
  const DINNER_BUFFER = 30;
  let cursor = routineStartMin;
  ...
  const steps = order.map((phaseKey, index) => {
    ...
    // If dinner conflicts with this routine and this is the first post-dinner phase,
    // skip cursor past the dinner buffer so post-dinner steps start after dinner ends
    ...
    if (dinnerConflict && !isBeforeDinner && prevWasBeforeDinner) {
      // Ensure cursor is at least at dinnerTime + buffer
      if (cursor < dinnerMin + DINNER_BUFFER) {
        cursor = dinnerMin + DINNER_BUFFER;
      }
    }
