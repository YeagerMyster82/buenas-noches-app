...
const phasePurpose = {
  ...
  bano_basico: "Cerrar lo practico antes de activar la calma, para evitar salir de la cama despues.",
  bano_regulacion:
...
const specialGuidance = {
  preparar_para_dormir: {
    ...
    examples: ["Bano o ducha tibia", "Ponerse pijama", "Bajar la luz", "Apagar pantallas", "Voz calmada"],
  },
  bano_basico: {
    title: "Bano y dientes",
    guidance: "Antes de activar la calma, hagan lo practico: cepillado de dientes e ir al bano por ultima vez.",
    examples: ["Cepillado de dientes", "Ir al bano por ultima vez"],
  },
  limite_claro: {
    title: "Limite claro",
    guidance:
...
export function buildPlan({
  ...
  const steps = order.map((phaseKey, index) => {
    ...
    if (phaseKey === "preparar_para_dormir") {
      const preparationItems = ["bath_or_shower", "put_on_pajamas", "dim_lights"].map((id) => activityCatalog[id]).filter(Boolean);
      return {
        id: stepId,
        phaseKey,
        label: phaseLabels[phaseKey],
