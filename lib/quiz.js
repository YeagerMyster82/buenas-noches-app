const profileContent = {
  es: {
    A: {
      name: "Segunda energia",
      description:
        "Este es el nino que parece enchufarse justo cuando tu ya estas agotada. Muchas veces no es que no tenga sueno. Es que llego tan cansado a la noche que su cuerpo saco un segundo aire para sostenerse un poco mas.",
    },
    B: {
      name: "Despierto pero quieto",
      description:
        "Este es el nino que ya esta acostado, no esta llorando, no esta pidiendo nada, pero tampoco se duerme. Se ve cansado, pero algo entre el cuerpo y el cerebro no termina de conectar.",
    },
    C: {
      name: "Negociador",
      description:
        "Este es el nino que siempre tiene una cosa mas. No lo hace para molestarte. Es una forma de estirar el tiempo de conexion porque su sistema nervioso todavia no esta listo para soltar el dia.",
    },
    D: {
      name: "Colapso nocturno",
      description:
        "Este es el nino que llega a la noche completamente sobrepasado. No es mala conducta. No es manipulacion. Es un sistema nervioso que acumulo demasiado durante el dia y llego a la noche sin recursos.",
    },
    E: {
      name: "Cuerpo inquieto",
      description:
        "Este es el nino que quiere dormir pero su cuerpo no coopera. Se mueve, gira, patea, cambia de posicion una y otra vez. Su cuerpo literalmente sigue buscando algo.",
    },
    F: {
      name: "Necesita cerquita",
      description:
        "Este es el nino que necesita sentirte para poder soltar. Tu presencia le funciona como senal de seguridad. Eso no es un defecto. Es biologia.",
    },
    G: {
      name: "Mente encendida",
      description:
        "Este es el nino que esta agotado pero su cabeza no para. Hace preguntas, recuerda cosas, quiere hablar y procesar. Cuanta mas presion siente, mas se activa.",
    },
    H: {
      name: "Se duerme, pero no sostiene el sueno",
      description:
        "Aqui la caida al sueno ocurre, pero la transicion al sueno profundo no se sostiene. Su sistema nervioso no termino de regularse completamente antes de dormir.",
    },
  },
  en: {
    A: {
...
const localizedQuestions = {
  es: [
    {
      id: 1,
      prompt: "Cuando llega la hora de dormir, que describe mejor a tu hijo?",
      options: [
    ...
    {
      id: 2,
      prompt: "Sientes que en la noche tiene mas energia de la normal?",
      options: [
        { key: "A", label: "Si, muchisimo" },
        { key: "B", label: "A veces" },
        { key: "C", label: "No" },
      ],
    },
    {
      id: 3,
      prompt: "Ya en la cama, que pasa mas seguido?",
      options: [
        { key: "A", label: "Se mueve mucho y no se queda quieto" },
        { key: "B", label: "Se queda quieto pero no se duerme" },
        { key: "C", label: "Sigue hablando, llamandote o pidiendo cosas" },
        { key: "D", label: "Se duerme, pero luego se despierta" },
      ],
    },
    {
      id: 4,
      prompt: "Con que frecuencia te pide una cosa mas antes de dormir?",
      options: [
        { key: "A", label: "Muchisimo" },
        { key: "B", label: "A veces" },
    ...
    {
      id: 5,
      prompt: "Necesita que estes cerca para poder dormirse?",
      options: [
        { key: "A", label: "Si, casi siempre" },
        { key: "B", label: "A veces" },
        { key: "C", label: "No" },
      ],
    },
    ...
    {
      id: 7,
      prompt: "En la noche se pone mas sensible, llora o explota emocionalmente?",
      options: [
        { key: "A", label: "Si, seguido" },
        { key: "B", label: "A veces" },
        { key: "C", label: "No" },
      ],
    },
    {
      id: 8,
      prompt: "Tu hijo se duerme pero se despierta 1 o 2 horas despues?",
      options: [
        { key: "A", label: "Si" },
        { key: "B", label: "A veces" },
        { key: "C", label: "No" },
    ...
    {
      id: 9,
      prompt: "Notas que su cuerpo esta inquieto en la cama?",
      options: [
        { key: "A", label: "Si" },
        { key: "B", label: "A veces" },
        { key: "C", label: "No" },
    ...
    {
      id: 10,
      prompt: "Parece calmarse mejor con presion o contacto fisico?",
      options: [
        { key: "A", label: "Si" },
...
export function buildResultCopy(result, language = "es") {
  ...
  return {
    ...
    primaryDescription: content[result.primary].description,
    reassurance:
      "Esto no es un problema de disciplina, no es que estes haciendo algo mal, y no es que tu hijo sea dificil. Es que su sistema nervioso todavia no esta recibiendo la senal correcta para soltar el dia y descansar.",
    framework:
      "Lo importante aqui es esto: cuando entiendes el perfil de tu hijo, dejas de adivinar. Dejas de probar cosas al azar. Y empiezas a trabajar con su sistema nervioso en lugar de contra el.",
    secondaryName: result.secondary ? content[result.secondary].name : null,
  };
}
