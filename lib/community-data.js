...
export async function listSupportMessagesForEmail(email) {
  ...
  const { data, error } = await supabase
    .from("support_messages")
    .select("id, parent_email, parent_name, child_name, topic, message, status, created_at, support_message_replies(id, sender, message, read_at, created_at)")
  ...
  return (data || []).map((message) => ({
    ...message,
    replies: (message.support_message_replies || []).sort((left, right) =>
      String(left.created_at).localeCompare(String(right.created_at))
    ),
    support_message_replies: undefined,
  }));
}
