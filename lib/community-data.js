...
export async function listPublicReviews() {
  ...
  const query = () =>
    supabase
      .from("app_reviews")
      .select("id, parent_name, child_name, rating, comment, photo_url, created_at")
...
export async function saveAppReview({
  ...
  if (error && /photo_url/i.test(error.message || "")) {
    const fallback = await supabase.from("app_reviews").insert(insertPayload).select().single();
    data = fallback.data;
    error = fallback.error;
  }
...
export async function getAdminDashboardData() {
  ...
  const [
    ...
    supabase
      ...
      .select("id, email, product_id, plan_type, subscription_status, is_active, price_usd, expires_at, renews_at, revenuecat_event_type, updated_at")
